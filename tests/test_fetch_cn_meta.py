...
def test_fetch_cn_meta_uses_gtimg_hero_map(monkeypatch):
    ...
    rows = fetch_cn_meta(role="mid", tier="diamond_plus")
...
def test_fetch_cn_meta_role_position_mapping_mid_adc_support(monkeypatch):
    ...
    mid_rows = fetch_cn_meta(role="mid", tier="diamond_plus")
    adc_rows = fetch_cn_meta(role="adc", tier="diamond_plus")
    support_rows = fetch_cn_meta(role="support", tier="diamond_plus")
...
def test_fetch_cn_meta_role_filtering_uses_row_position_without_rotation(monkeypatch):
    ...
    assert fetch_cn_meta(role="top", tier="diamond_plus")[0]["hero_id"] == "10002"
    assert fetch_cn_meta(role="jungle", tier="diamond_plus")[0]["hero_id"] == "10005"
    assert fetch_cn_meta(role="mid", tier="diamond_plus")[0]["hero_id"] == "10001"
    assert fetch_cn_meta(role="adc", tier="diamond_plus")[0]["hero_id"] == "10003"
    assert fetch_cn_meta(role="support", tier="diamond_plus")[0]["hero_id"] == "10004"
...
def test_summarize_cn_positions_groups_lane_distribution(monkeypatch):
    ...
    payload = summarize_cn_positions(tier="monarch")

    assert payload["tier"] == "monarch"
